...
def test_get_user_credit_limit_returns_active_non_expired(
...
def test_get_user_credit_limit_ignores_expired(
        user, active_credit_limit_factory
):
...
def test_get_available_credit_matches_property(
        user, active_credit_limit_factory, current_statement_factory
):
    limit = active_credit_limit_factory(
...
def test_available_limit_uses_only_current_debt_ignores_pending(
        user, active_credit_limit_factory
):
    limit = active_credit_limit_factory(
...
def test_available_limit_is_clamped_to_zero_when_carryover_exceeds_limit(
        user, active_credit_limit_factory, current_statement_factory
):
    limit = active_credit_limit_factory(
...
def test_grace_days_override(user, active_credit_limit_factory):
    limit = active_credit_limit_factory(
        user=user, grace_days=7
    )  # factory should map to grace_period_days
    assert limit.grace_days == 7


def test_grace_days_default_follows_settings(
        monkeypatch, user, active_credit_limit_factory
):
...
def test_db_unique_constraint_allows_only_one_active_per_user(
        user, active_credit_limit_factory
):
...
def test_reference_code_is_generated(user, active_credit_limit_factory):
    ...
    )  # avoid unique-active constraint interplay
...
def test_str_contains_user_and_formatted_amount(
        user, active_credit_limit_factory
):
    limit = active_credit_limit_factory(
...
def test_get_user_credit_limit_excludes_expiring_today(
        user, active_credit_limit_factory, monkeypatch
):
    # Force 'today' to be deterministic if needed, but using real localdate is fine here
    limit = active_credit_limit_factory(
        user=user, is_active=True, expiry_days=0
    )
    assert CreditLimit.objects.get_user_credit_limit(user) is None


def test_available_limit_reflects_purchase_and_payment(
        user, active_credit_limit_factory, current_statement_factory
):
    limit = active_credit_limit_factory(
        user=user, approved_limit=1_000_000, is_active=True
    ...
    stmt.refresh_from_db()
    assert limit.available_limit == 850_000  # 1_000_000 - (200_000 - 50_000)


def test_reference_code_five_collisions_then_null(monkeypatch, user):
    from credit.models import credit_limit as cl_mod
    def dup_gen(prefix="CR"): return "CR-DUP"

    monkeypatch.setattr(cl_mod, "generate_reference_code", dup_gen)
    # Pre-create one with CR-DUP to ensure collisions
    CreditLimit.objects.create(
    ...
    obj.save()
    assert obj.reference_code is None  # fell back to saving with NULL
