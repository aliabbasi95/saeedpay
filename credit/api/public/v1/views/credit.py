...
class StatementListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Statement.objects.filter(user=user).order_by('-year', '-month', '-created_at')
...
class StatementLineListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = StatementLine.objects.filter(statement__user=user).select_related('statement')
    ...
    def post(self, request):
        ...
        try:
            ...
            if not statement:
                ...
                statement, _ = Statement.objects.get_or_create_current_statement(user, starting_balance=starting_balance)
...
class AddPaymentView(APIView):
    ...
    def post(self, request):
        ...
        if transaction_id:
            ...
            if hasattr(transaction, 'status') and transaction.status != TransactionStatus.SUCCESS:
                return Response({'error': 'invalid or unsuccessful transaction'}, status=400)
            if not (transaction.from_wallet.user_id == user.id or transaction.to_wallet.user_id == user.id):
                return Response({'error': 'transaction does not belong to user'}, status=400)
...
class ApplyPenaltyView(APIView):
    """
    Apply penalty to overdue statement.
    
    This view calculates and applies penalty charges to the user's latest pending
    or overdue statement. Penalties are calculated based on the overdue amount
    ...
    def post(self, request):
        user = request.user
        # Target latest pending statement that is overdue
        statement = Statement.objects.filter(user=user, status__in=['pending_payment', 'overdue']).order_by('-year', '-month').first()
        if not statement:
            return Response({'error': 'No pending or overdue statement'}, status=400)
