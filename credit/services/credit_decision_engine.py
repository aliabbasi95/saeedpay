...
class CreditDecisionEngine:
    ...
    def _make_decision(
        ...
        if requested_amount > approved_limit:
            return {
                ...
                "risk_score": risk_score,
                "message": (
                    f"مبلغ درخواستی تعدیل شد. مبلغ تاییدشده: {approved_limit:,} " "ریال"
                ),
            }
    ...
    @transaction.atomic
    def approve_credit_limit(self, credit_limit_id: int) -> Dict[str, Any]:
        """Approve a pending credit limit and activate it"""
        try:
            credit_limit = CreditLimit.objects.get(id=credit_limit_id)
            credit_limit.approve_and_activate()

            # Create initial statement for the user after approval
            Statement.objects.create_initial_statement(credit_limit.user)

            return {
                "success": True,
                "message": "حد اعتباری با موفقیت تایید و فعال شد",
                "credit_limit_id": credit_limit.id,
            }
        except CreditLimit.DoesNotExist:
            return {"success": False, "message": "حد اعتباری یافت نشد"}
    ...
    def reject_credit_limit(
        ...
        try:
            credit_limit = CreditLimit.objects.get(id=credit_limit_id)
            credit_limit.reject(reason)

            return {
                "success": True,
                "message": "حد اعتباری رد شد",
                "credit_limit_id": credit_limit.id,
            }
        except CreditLimit.DoesNotExist:
            return {"success": False, "message": "حد اعتباری یافت نشد"}
