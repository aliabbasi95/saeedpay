...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--user_id", type=int, help="User ID for specific user tasks")

        parser.add_argument(
            "--async", action="store_true", help="Run tasks asynchronously"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Testing Celery tasks: {task_name}"))
    ...
    def test_month_end_task(self, run_async=False):
        ...
        if run_async:
            ...
            self.stdout.write(self.style.SUCCESS(f"Month-end task queued: {task.id}"))
        else:
