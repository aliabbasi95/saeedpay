...
class VideoVerificationSubmitSerializer(serializers.Serializer):
    ...
    national_id = serializers.CharField(
        max_length=10, required=True, help_text="Iranian national ID (10 digits)"
    )
    birth_date = serializers.CharField(
        max_length=10, required=True, help_text="Birth date in format YYYY/MM/DD"
    )
    selfie_video = serializers.FileField(
        required=True, help_text="Selfie video file for verification"
    )
    rand_action = serializers.CharField(
        max_length=50, required=True, help_text="Random action string for verification"
    )
    matching_thr = serializers.IntegerField(
        required=False, help_text="Facial matching threshold (optional)"
    )
    liveness_thr = serializers.IntegerField(
        required=False, help_text="Liveness detection threshold (optional)"
    )
    ...
    def validate_selfie_video(self, value):
        name_ok = any(value.name.lower().endswith(ext) for ext in _ALLOWED_EXTS)
