...
class TestSendOtpThrottlingTests:
    ...
    def test_throttle_configuration_verification(self):
        ...
        otp_throttle_found = any(isinstance(throttle, OTPPhoneRateThrottle) for throttle in throttle_classes)
        ...
        assert config["requests_allowed"] > 0, "Throttle should allow at least 1 request"
        assert config["period_seconds"] > 0, "Throttle period should be positive"
        
        print(f"✓ Throttle configured: {config['requests_allowed']} requests per {config['period_seconds']} seconds ({config['rate_string']}).")
    ...
    def test_phone_throttle_allows_up_to_limit(self):
        ...
        time_between_requests = max(1, config["period_seconds"] // requests_allowed // 2)
        ...
        with freeze_time("2024-01-01 12:00:00") as frozen_time:
            ...
            with patch('auth_api.models.PhoneOTP.send', return_value=True):
                ...
                for i in range(requests_allowed):
                    ...
                    frozen_time.tick(delta=timezone.timedelta(seconds=time_between_requests))
                
                # Next request should be throttled
                response = self.client.post(SEND_OTP_URL, {"phone_number": phone})
                assert response.status_code == status.HTTP_429_TOO_MANY_REQUESTS, f"Request {requests_allowed + 1} should be throttled but got {response.status_code}: {response.data}"
    ...
    def test_phone_throttle_resets_after_period(self):
        ...
        with freeze_time("2024-01-01 12:00:00") as frozen_time:
            with patch('auth_api.models.PhoneOTP.send', return_value=True):
                ...
                # Move forward but still within the period - should still be throttled  
                frozen_time.tick(delta=timezone.timedelta(seconds=period_seconds // 2))
                response = self.client.post(SEND_OTP_URL, {"phone_number": phone})
                assert response.status_code == status.HTTP_429_TOO_MANY_REQUESTS
                
                # Move past the period from last successful request - should reset
                frozen_time.tick(delta=timezone.timedelta(seconds=period_seconds // 2 + 5))
                response = self.client.post(SEND_OTP_URL, {"phone_number": phone})
                assert response.status_code == status.HTTP_200_OK
    ...
    def test_phone_throttle_simple_reset(self):
        ...
        with freeze_time("2024-01-01 12:00:00") as frozen_time:
            with patch('auth_api.models.PhoneOTP.send', return_value=True):
                ...
                frozen_time.tick(delta=timezone.timedelta(seconds=period_seconds + 1))
                response = self.client.post(SEND_OTP_URL, {"phone_number": phone})
                assert response.status_code == status.HTTP_200_OK
    ...
    def test_phone_throttle_per_phone_isolation(self):
        ...
        with freeze_time("2024-01-01 12:00:00") as frozen_time:
            with patch('auth_api.models.PhoneOTP.send', return_value=True):
                ...
                response = self.client.post(SEND_OTP_URL, {"phone_number": phone2})
                assert response.status_code == status.HTTP_200_OK
    ...
    def test_model_level_throttle_vs_drf_throttle_interaction(self):
        ...
        with freeze_time("2024-01-01 12:00:00") as frozen_time:
            ...
            frozen_time.tick(delta=timezone.timedelta(minutes=4))  # Total: 6 minutes
            response = self.client.post(SEND_OTP_URL, {"phone_number": phone})
            assert response.status_code == status.HTTP_200_OK
    ...
    def test_dynamic_throttle_rate_compatibility(self):
        ...
        print(f"   Limit: {config['requests_allowed']} requests per {config['period_seconds']} seconds")
        ...
        with freeze_time("2024-01-01 12:00:00") as frozen_time:
            with patch('auth_api.models.PhoneOTP.send', return_value=True):
                ...
                print(f"✅ Successfully tested {config['requests_allowed']} allowed requests + throttling")
