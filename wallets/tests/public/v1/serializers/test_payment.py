...
class TestPaymentRequestCreateSerializer:
    ...
    def test_valid_data(self):
        data = {"amount": 1000, "return_url": "https://callback.com"}
        s = PaymentRequestCreateSerializer(data=data)
        assert s.is_valid()
        assert s.validated_data["amount"] == 1000
        assert s.validated_data["return_url"] == "https://callback.com"
    ...
    @pytest.mark.parametrize("val", [None, "https://"])
    def test_edge_case_urls(self, val):
        s = PaymentRequestCreateSerializer(
            data={"amount": 5, "return_url": val}
        )
        assert not s.is_valid()
        assert "return_url" in s.errors
