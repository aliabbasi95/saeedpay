...
class TestPaymentRequestModel:
    ...
    def test_same_status_transition_is_noop(self, store):
        pr = PaymentRequest.objects.create(
            store=store,
            amount=2000,
        ...
        first_completed_at = pr.completed_at
        first_cancelled_at = pr.cancelled_at
        first_expired_at = pr.expired_at
        first_updated_at = pr.updated_at

        pr.mark_awaiting_merchant()
        pr.refresh_from_db()

        assert pr.status == first_status
        assert pr.completed_at == first_completed_at
        assert pr.cancelled_at == first_cancelled_at
        assert pr.expired_at == first_expired_at
        assert pr.updated_at == first_updated_at
    ...
    def test_paid_by_and_paid_wallet_fields(
        ...
        pr = PaymentRequest.objects.create(
            store=store,
            amount=700,
            return_url="https://paid.com",
            paid_by=customer_user,
            paid_wallet=customer_cash_wallet,
        )
