...
class CommentViewSet(ReCaptchaMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Comment.objects.select_related('author', 'article', 'reply_to').prefetch_related('replies')
    ...
    def my_comments(self, request):
        ...
        queryset = Comment.objects.filter(author=request.user).select_related('article', 'reply_to')
    ...
    def article_comments(self, request):
        ...
        if not article_id:
            return Response({'detail': 'article_id parameter is required'}, status=status.HTTP_400_BAD_REQUEST)
        
        # Only get root comments (no reply_to) - replies are included in serializer
        queryset = self.get_queryset().filter(article_id=article_id, reply_to__isnull=True)
        page = self.paginate_queryset(queryset)
        if page is not None:
    ...
    def dislike(self, request, pk=None):
        ...
        comment = self.get_object()
        Comment.objects.filter(pk=comment.pk).update(dislike_count=F('dislike_count') + 1)
        comment.refresh_from_db(fields=['like_count', 'dislike_count'])
        return Response({
            'id': comment.pk,
            'like_count': comment.like_count,
