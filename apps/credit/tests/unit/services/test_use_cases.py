...
class TestPurchases:
    def test_record_successful_purchase_creates_purchase_line_and_updates_balances(
        ...
        assert stmt.closing_balance == stmt.opening_balance + 0 - abs(line.amount)
    ...
    def test_record_successful_purchase_rejects_when_tx_not_belonging_to_user(
        ...
        alien = django_user_model.objects.create(username="alien", password="x")
        w_from = Wallet.objects.create(
            user=alien, owner_type=OwnerType.CUSTOMER, kind=WalletKind.CREDIT, balance=0
        )
        w_to = Wallet.objects.create(
            user=alien,
            owner_type=OwnerType.MERCHANT,
        ...
        with pytest.raises(ValueError):
            StatementUseCases.record_successful_purchase_from_transaction(alien_tx.id)
    ...
    def test_record_successful_purchase_rejects_when_credit_limit_inactive_or_expired(
        ...
        w_from, w_to = wallets_for_user
        tx = Transaction.objects.create(
            from_wallet=w_from, to_wallet=w_to, amount=90_000, status=TxS.SUCCESS
        )
        ...
        with pytest.raises(ValueError, match="No active credit limit"):
            StatementUseCases.record_successful_purchase_from_transaction(tx.id)
...
class TestPayments:
    ...
    def test_record_payment_rejects_zero_amount(self, user):
        with pytest.raises(ValueError, match="non-zero"):
            StatementUseCases.record_payment_on_current_statement(user, amount=0)
...
class TestMonthEndRollover:
    def test_month_end_rollover_closes_past_current_and_creates_new_with_interest_for_negative_carryover(
        ...
        old = Statement.objects.get(user=user, year=past_year, month=past_month)
        assert old.status == StatementStatus.PENDING_PAYMENT
        new_current = Statement.objects.get(user=user, status=StatementStatus.CURRENT)
        assert new_current.opening_balance == old.closing_balance
        assert new_current.lines.filter(type=StatementLineType.INTEREST).exists()
    ...
    def test_month_end_rollover_no_interest_when_carryover_non_negative(
        ...
        assert result["statements_closed"] >= 1
        new_current = Statement.objects.get(user=user, status=StatementStatus.CURRENT)
        assert not new_current.lines.filter(type=StatementLineType.INTEREST).exists()
    ...
    def test_month_end_rollover_is_idempotent_does_not_duplicate_interest(
        ...
        current1 = Statement.objects.get(user=user, status=StatementStatus.CURRENT)
        ...
        StatementUseCases.perform_month_end_rollover()
        current2 = Statement.objects.get(user=user, status=StatementStatus.CURRENT)
    ...
    def test_month_end_rollover_updates_existing_current_and_adds_single_interest(
        ...
        _cur_existing, _ = Statement.objects.get_or_create_current_statement(user)

        # Act
        StatementUseCases.perform_month_end_rollover()

        # --- Re-fetch fresh instances from DB (DO NOT reuse cached objects) ---
        past_after = Statement.objects.get(user=user, year=prev_year, month=prev_month)
        current_after = Statement.objects.get(user=user, status=StatementStatus.CURRENT)
        ...
        assert current_after.lines.filter(type=StatementLineType.INTEREST).count() == 1
    ...
    def test_rollover_interest_amount_exact(
        ...
        StatementUseCases.perform_month_end_rollover()
        current = Statement.objects.get(user=user, status=StatementStatus.CURRENT)
    ...
    def test_month_end_rollover_counts_multiple_users(
        ...
        user_b = django_user_model.objects.create(username="user_b", password="x")
        active_credit_limit_factory(user=user, is_active=True, expiry_days=30)
        active_credit_limit_factory(user=user_b, is_active=True, expiry_days=30)
        ...
        curr_a = Statement.objects.get(user=user, status=StatementStatus.CURRENT)
        curr_b = Statement.objects.get(user=user_b, status=StatementStatus.CURRENT)
...
class TestFinalizeDueWindows:
    def _make_pending_with_due(
        self, user, closing_balance=-1_000_000, grace_days=5, closed_at=None
    ):
        """
        ...
        if delta > 0:
            stmt.add_line(StatementLineType.PURCHASE, delta, description="seed debt")
            stmt.refresh_from_db()
    ...
    def test_finalize_due_windows_no_penalty_when_minimum_paid_within_window(
        ...
        try:
            ...
            current.add_line(
                StatementLineType.PAYMENT, min_required, description="on-time payment"
            )
        finally:
        ...
        assert not current.lines.filter(type=StatementLineType.PENALTY).exists()
    ...
    def test_finalize_due_windows_creates_current_if_missing(
        ...
        assert (
            Statement.objects.filter(user=user, status=StatementStatus.CURRENT).count()
            == 0
        )
    ...
    def test_finalize_due_windows_penalty_is_capped_by_max_rate(
        ...
        active_credit_limit_factory(user=user, is_active=True, expiry_days=30)
        debt = -5_000_000
        self._make_pending_with_due(user, closing_balance=debt, grace_days=1)
        current, _ = Statement.objects.get_or_create_current_statement(user)
    ...
    def test_finalize_due_windows_no_penalty_when_pending_balance_non_negative(
        ...
        if stmt.closing_balance < 0:
            stmt.add_line(StatementLineType.PAYMENT, abs(int(stmt.closing_balance)) + 1)
    ...
    def test_finalize_due_windows_is_idempotent_no_duplicate_penalty(
        ...
        self._make_pending_with_due(user, closing_balance=-1_200_000, grace_days=2)
        current, _ = Statement.objects.get_or_create_current_statement(user)

        now = timezone.localtime(timezone.now()) + dt.timedelta(days=10)
        # First run
        StatementUseCases.finalize_due_windows(now=now)
        pen_count_1 = current.lines.filter(type=StatementLineType.PENALTY).count()

        # Second run — nothing new should be added
        StatementUseCases.finalize_due_windows(now=now + dt.timedelta(days=1))
        pen_count_2 = current.lines.filter(type=StatementLineType.PENALTY).count()

        assert pen_count_1 in (
            0,
            1,
        )  # penalty may be zero if capped/threshold; accept both shapes
    ...
    def test_finalize_due_windows_multiple_partial_payments_cover_minimum(
        ...
        try:
            ...
            current.add_line(StatementLineType.PAYMENT, part1, description="part1")
            timezone.now = lambda: end - dt.timedelta(seconds=1)
            current.add_line(StatementLineType.PAYMENT, part2, description="part2")
        finally:
            timezone.now = orig_now

        res = StatementUseCases.finalize_due_windows(now=end + dt.timedelta(seconds=1))
        assert res.finalized_count >= 1
        pending.refresh_from_db()
        assert pending.status == StatementStatus.CLOSED_NO_PENALTY
        assert not current.lines.filter(type=StatementLineType.PENALTY).exists()
    ...
    def test_finalize_noop_when_no_pending_past_due(
        ...
        assert (
            res.finalized_count,
            res.closed_without_penalty_count,
            res.closed_with_penalty_count,
        ) == (0, 0, 0)
...
class TestSumPaymentsHelper:
    ...
    def test_sum_payments_includes_end_boundary(self, user, current_statement_factory):
        stmt = current_statement_factory(user)
        t0 = timezone.localtime(timezone.now())
        orig_now = timezone.now
        try:
    ...
    def test_sum_payments_on_current_during_window_returns_zero_for_invalid_window_or_missing_bounds(
        ...
        s1 = StatementUseCases._sum_payments_on_current_during_window(
            current_statement=stmt, start=now, end=now - dt.timedelta(seconds=1)
        )
        # missing bounds → zero
...
class TestFinalizeCountsMultiUser:
    def test_finalize_due_windows_counts_multiple_users_split_penalty_and_no_penalty(
        ...
        """
        # Create another user
        user_b = django_user_model.objects.create(username="buyer_b", password="x")
        active_credit_limit_factory(user=user, is_active=True, expiry_days=30)
        active_credit_limit_factory(user=user_b, is_active=True, expiry_days=30)
        ...
        TestFinalizeDueWindows()._make_pending_with_due(
...
class TestSumIgnoresNonPayments:
    def test_sum_payments_ignores_non_payment_lines_inside_window(
        ...
        s = StatementUseCases._sum_payments_on_current_during_window(
            current_statement=stmt,
            start=t0 - dt.timedelta(minutes=1),
