...
class TestCloseMonthlyStatements:
    ...
    def test_is_idempotent(self, user):
        ...
        assert result2["interest_lines_added"] in (0, 1)  # typically 0 on rerun
    ...
    def test_updates_existing_current_opening_balance(
        ...
        expected_interest = int(abs(prev_stmt.closing_balance) * MONTHLY_INTEREST_RATE)
        assert (
            existing_current.lines.filter(
                type=StatementLineType.INTEREST, amount=-expected_interest
            ).count()
            == 1
        )
    ...
    def test_interest_line_description_format(self, user):
        ...
        assert line is not None and f"{py}/{pm:02d}" in (line.description or "")
    ...
    def test_no_interest_when_prev_non_negative(
        ...
        assert (
            ...
            and res["statements_created"] == 1
            and res["interest_lines_added"] == 0
        )
        today = JalaliDate.today()
        current = Statement.objects.get(
            user=user,
    ...
    def test_existing_current_does_not_duplicate_interest(
        ...
        Statement.objects.close_monthly_statements()
        cur.refresh_from_db()
        second_count = cur.lines.filter(type=StatementLineType.INTEREST).count()
