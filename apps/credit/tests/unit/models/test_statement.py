...
class TestClosingStatement:
    def test_sets_pending_and_due_date(self, user, active_credit_limit_factory):
        ...
        assert stmt.due_date is not None and (stmt.due_date - stmt.closed_at).days == 10
...
class TestDueWindowOutcome:
    def test_no_penalty_when_enough_payment(self, user):
        ...
        outcome = stmt.determine_due_outcome(total_payments_during_due=min_required)
        assert (
            outcome == StatementStatus.CLOSED_NO_PENALTY and stmt.closed_at is not None
        )
    ...
    def test_with_penalty_when_insufficient_payment(self, user):
        ...
        stmt.closing_balance = -1_000_000
        min_required = stmt.calculate_minimum_payment_amount()
        outcome = stmt.determine_due_outcome(total_payments_during_due=min_required - 1)
        assert (
            outcome == StatementStatus.CLOSED_WITH_PENALTY
            and stmt.closed_at is not None
        )
...
class TestPenaltyComputation:
    ...
    def test_uses_explicit_now(self, user):
        ...
        assert stmt.compute_penalty_amount(now=custom_now) == min(expected_raw, cap)
...
class TestInterestCarryOverHelper:
    def test_adds_line_only_on_negative_and_current(self, user):
        ...
        assert (
            current_stmt.lines.filter(type=StatementLineType.INTEREST).count() == 1
        )  # unchanged
...
class TestAddPayment:
    def test_success_creates_positive_payment_line(self, user):
        ...
        stmt.refresh_from_db()
        assert (
            stmt.lines.filter(type=StatementLineType.PAYMENT, amount=5_000).count() == 1
        )
    ...
    def test_accepts_negative_and_normalizes_to_positive(self, user):
        ...
        stmt.refresh_from_db()
        assert (
            stmt.lines.filter(type=StatementLineType.PAYMENT, amount=3_000).count() == 1
        )
...
class TestAddPurchase:
    ...
    def test_raises_on_non_current(self, user, monkeypatch):
        ...
        with pytest.raises(ValueError):
            stmt.add_purchase(
                self._make_dummy_tx(user.id, 10_000, status_ok=True, belong="both")
            )
    ...
    def test_raises_on_non_success_tx(self, user, monkeypatch):
        ...
        with pytest.raises(ValueError):
            stmt.add_purchase(
                self._make_dummy_tx(user.id, 10_000, status_ok=False, belong="both")
            )
    ...
    def test_raises_when_tx_not_belongs_to_user(self, user, monkeypatch):
        ...
        with pytest.raises(ValueError):
            stmt.add_purchase(
                self._make_dummy_tx(user.id, 10_000, status_ok=True, belong="none")
            )
    ...
    def test_raises_on_insufficient_available_limit(self, user, monkeypatch):
        ...
        with pytest.raises(ValueError):
            stmt.add_purchase(
                self._make_dummy_tx(user.id, 10_001, status_ok=True, belong="both")
            )
    ...
    def test_success_calls_add_line_with_abs_amount(self, user, monkeypatch):
        ...
        monkeypatch.setattr(
            CreditLimit.objects,
            "get_user_credit_limit",
            lambda u: self._stub_credit_limit(available=999_999),
        )
        called = {}
        ...
        trx = self._make_dummy_tx(user.id, amount=12_345, status_ok=True, belong="both")
